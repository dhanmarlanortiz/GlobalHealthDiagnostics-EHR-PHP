...
<div class="flex min-w-0 gap-x-4">
    <div class="min-w-0 flex-auto">
        <p class="text-sm font-semibold leading-6 text-gray-900">Gynecologic Report</p>
        <p class="text-xs leading-5 text-gray-500 flex gap-x-4">';
            if($gynecologicReport) {
                ...
                echo    date("M d, Y", strtotime($gynecologicReport['gynerep_date']));
                echo '</span>';
            } else {
                echo 'Not Available';
            }
...
<div class="shrink-0 mt-2 sm:mt-0 sm:flex sm:items-end">';
    if($gynecologicReport) {
        $gynecologicReportData = [
            'href' => '/reports/gynecologic-report.php?id=' . $id,
            'organizationId' => $o,
            'firstName' => $_POST['firstName'],
            'middleName' => $_POST['middleName'],
            'lastName' => $_POST['lastName']
        ];
        $encodeGynecologicReport = base64_encode(json_encode( $gynecologicReportData ));

        echo "<a href='" . base_url(false) . "/employeeViewGynecologicReport-APE.php?q=$encodeGynecologicReport' title='View' class='btn btn-sm bg-green-700 hover:bg-green-800'>
        ...
        echo "<a href='" . base_url(false) . "/reports/gynecologic-report.php?id=" . $id . "' title='Download' class='btn btn-sm ml-1 bg-sky-400 hover:bg-sky-500' download>
        ...
        if($role == 1 || $role == 3) {
            echo "<button type='button' onClick='gynecologicReportModal.showModal()' title='Edit' class='btn btn-sm ml-1 bg-amber-500 hover:bg-amber-600 text-white'>
            ...
            echo "<button type='submit' name='deleteGynecologicReport' title='delete' class='btn btn-sm ml-1 bg-red-500 hover:bg-red-600 text-white'>
                <span class='sr-only'>Delete Gynecologic Report</span>
        ...
        if($role == 1 || $role == 3) {
            echo "<button type='button' onClick='gynecologicReportModal.showModal()' title='Create' class='btn btn-sm bg-green-700 hover:bg-green-800'>
